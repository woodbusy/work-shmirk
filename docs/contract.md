...
- `symlink_dir` path expansion recognizes exactly three leading-token forms: `~/...` or bare `~`, `$HOME/...` or bare `$HOME`, and `${HOME}/...` or bare `${HOME}`. All three expand to the value of `$HOME`. A `~` not immediately followed by `/` or end-of-string (e.g. `~alice/`) is rejected with an error — `~user`-style home-dir lookup is not supported. Mid-string occurrences of any of these tokens are left literal.
- `symlink_links` strips **all** leading `.` characters from each entry name. e.g. `.env` → `env`, `..env` → `env`, `...env` → `env`. Entries whose stripped form is empty (`.`, `..`, `...`, etc.) and raw empty entries (`""`) are rejected with an error rather than silently skipped.
